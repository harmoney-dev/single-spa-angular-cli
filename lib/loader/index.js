...
var unload = function (opts, props) {
    return new Promise(function (resolve, reject) {
        var scriptsPromise = opts.scripts.reduce(function (prev, fileName) { return prev.then(unloadTag(opts.baseHref + "/" + fileName)); }, Promise.resolve(undefined));
        var stylesPromise = opts.styles.reduce(function (prev, fileName) { return prev.then(unloadTag(opts.baseHref + "/" + fileName)); }, Promise.resolve(undefined));
        Promise.all([scriptsPromise, stylesPromise]).then(resolve, reject);
    });
};
